...
for tf in (1, 5):
    ...
    for name, sig in PRESETS.items():
        r = run(d=d, one_per_cross=True, useDD=True, reset_on_stall=True, **sig)
        a = run(d=d[d.index < mid], one_per_cross=True, useDD=False, **sig)
        b = run(d=d[d.index >= mid], one_per_cross=True, useDD=False, **sig)
