...
Only complete outcomes are used (entries older than the 6-hour follow-up), so the newest data is not biased toward quick exits. The search needs at least a day of recorded market and 1,000 finished would-be trades before it answers, runs every 2 hours (and after every learning cycle), and never changes settings on its own: **Paper-trade this rule** is a button, and Telegram's `/edges` shows its latest answer on the phone.
